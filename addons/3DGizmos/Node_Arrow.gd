...
class_name GzmoArrow extends Node3D
...
func _ready() -> void:
	if Engine.is_editor_hint():
		myMesh.mesh = preload('res://addons/3DGizmos/Msh_Arrow.obj')
...
# LIBRARY BY CODE
func Set3DGizmOptions ( Orign : Vector3, LookAt : Vector3, Size : float = 1.0, LifeTime : float = 0) :
	position = Orign
	look_at(LookAt, Vector3.UP, true)
