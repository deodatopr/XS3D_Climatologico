...
@onready var current_button = preload("res://addons/DatePickerFiles/CalendarPop/CurrentButton.tscn")
@onready var past_button = preload("res://addons/DatePickerFiles/CalendarPop/PastButton.tscn")
@onready var future_button = preload("res://addons/DatePickerFiles/CalendarPop/FutureButton.tscn")
@onready var year_button = preload("res://addons/DatePickerFiles/CalendarPop/year_button.tscn")
...
@onready var hour_picker = %HourPicker
@onready var first_hour_bttn = $PopLayer/HourPicker/ScrollContainer/VBoxContainer/Button
@onready var hour_bttns_container = $PopLayer/HourPicker/ScrollContainer/VBoxContainer

var year = 2024
var month = 1
var day = 1

var Hour_Bttn_Selected : Button
var Hour_Bttn_Pressed : Button
...
func _ready():
	...
	global_date_data.hour = current_date_string.hour
	
	Hour_Bttn_Selected = first_hour_bttn
	Hour_Bttn_Selected.button_pressed = true
	Hour_Bttn_Pressed = Hour_Bttn_Selected
	...
	for bttn in hour_bttns_container.get_children():
		var bttnHour = bttn.text.split(":")
		if not(global_date_data["year"] == current_date_string["year"] and global_date_data["month"] == current_date_string["month"] and global_date_data["day"] == current_date_string["day"] and int(bttnHour[0]) <= current_date_string["hour"]):
			bttn.disabled = true
		bttn.pressed.connect(self._selected_bttn.bind(bttn))

func _selected_bttn(_bttn : Button):
	for bttn in hour_bttns_container.get_children():
		bttn.button_pressed = false
		
	Hour_Bttn_Pressed = _bttn
	Hour_Bttn_Pressed.button_pressed = true
	
	var hour = Hour_Bttn_Pressed.text.split(":")
	selectedHour = int(hour[0])
	
	var currentDate = Time.get_datetime_dict_from_system()
	if not(global_date_data["year"] == currentDate["year"] and global_date_data["month"] == currentDate["month"] and global_date_data["day"] == currentDate["day"] and selectedHour > currentDate["hour"]):
		var new_selected_date = str(get_weekday_name(global_date_data.weekday)) + ", " + str(get_month_name(global_date_data.month)) + " " + str(global_date_data.day) + ", " + str(global_date_data.year) + " - " + Hour_Bttn_Pressed.text
...
func update_month_year_label(year, month):
	var month_abbreviations = ["", "En", "Feb", "Mar", "Abr", "May", "Jun", "Jul", "Agto", "Sept", "Oct", "Nov", "Dic"]
...
func load_the_date(param_year, param_month):
	...
	for bttn in hour_bttns_container.get_children():
		var bttnHour = bttn.text.split(":")
		if param_year > currentDate["year"]:
			bttn.disabled = true
		elif param_month > currentDate["month"] and param_year == currentDate["year"]:
			bttn.disabled = true
		elif int(bttnHour[0]) > currentDate["hour"] and param_month == currentDate["month"] and param_year == currentDate["year"]:
			bttn.disabled = true
		else:
			bttn.disabled = false
...
func generate_current_dates(date_data, param_month, param_year):
	for i in date_data.size():
		...
		var start_weekday = Time.get_datetime_dict_from_datetime_string(the_target_date, true)
		var stringdate = the_target_date.split("T")
		var checkdate = stringdate[0].split("-")
		if int(checkdate[0]) > Time.get_datetime_dict_from_system().year:
			target_button.disabled = true
			target_button.toggle_mode = false
		elif int(checkdate[1]) > Time.get_datetime_dict_from_system().month and int(checkdate[0]) == Time.get_datetime_dict_from_system().year:
			target_button.disabled = true
			target_button.toggle_mode = false
		elif int(checkdate[2]) > Time.get_datetime_dict_from_system().day and int(checkdate[1]) == Time.get_datetime_dict_from_system().month and int(checkdate[0]) == Time.get_datetime_dict_from_system().year:
			target_button.disabled = true
			target_button.toggle_mode = false
		else:
			target_button.date_pressed.connect(self.update_date_pressed)
...
func _on_button_pressed():
	...
	if date_picker_panel.visible:
		...
		hour_picker.position.x = new_pos.x + date_picker_panel.size.x
		hour_picker.position.y = new_pos.y
		choose_month_year.position = new_pos
		
		var currentDate = Time.get_datetime_dict_from_system()
		for bttn in hour_bttns_container.get_children():
			var bttnHour = bttn.text.split(":")
			if final_date_data["day"] == currentDate["day"] and int(bttnHour[0]) >= currentDate["hour"]:
				bttn.disabled = true
			elif final_date_data["day"] != currentDate["day"]:
				bttn.disabled = false
			bttn.button_pressed = false
		Hour_Bttn_Selected.button_pressed = true
...
func update_date_pressed(button_node, button_type, button_data):
	...
	var hour = global_date_data.hour
	global_date_data = button_data
	global_date_data.hour = hour
	var new_selected_date = str(get_weekday_name(button_data.weekday)) + ", " + str(get_month_name(button_data.month)) + " " + str(button_data.day) + ", " + str(button_data.year) + " - " + Hour_Bttn_Pressed.text
	selected_date_label.text = new_selected_date
	
	var currentDate = Time.get_datetime_dict_from_system()
	for bttn in hour_bttns_container.get_children():
		var bttnHour = bttn.text.split(":")
		if global_date_data["year"] > currentDate["year"]:
			bttn.disabled = true
		elif global_date_data["month"] > currentDate["month"] and global_date_data["year"] == currentDate["year"]:
			bttn.disabled = true
		elif global_date_data["day"] > currentDate["day"] and global_date_data["month"] == currentDate["month"] and global_date_data["year"] == currentDate["year"]: 
			bttn.disabled = true
		elif int(bttnHour[0]) > global_date_data["hour"] and global_date_data["day"] == currentDate["day"] and global_date_data["month"] == currentDate["month"] and global_date_data["year"] == currentDate["year"]:
			bttn.disabled = true
		else:
			bttn.disabled = false
...
func select_month(our_month):
	month_grid.get_child(our_month-1).button_pressed = true
...
func _on_cancel_pressed():
	hour_picker.visible = false
	choose_month_year.visible = false
	select_day(final_date_data["day"])
	date_picker_panel.visible = false

func _on_confirm_pressed():
	var currentDate = Time.get_datetime_dict_from_system()
	if not(global_date_data["year"] == currentDate["year"] and global_date_data["month"] == currentDate["month"] and global_date_data["day"] == currentDate["day"] and selectedHour > currentDate["hour"]):
		Hour_Bttn_Selected = Hour_Bttn_Pressed
	hour_picker.visible = false
	choose_month_year.visible = false
	final_date_data = global_date_data
	date_picker_button.text = selected_date_label.text
